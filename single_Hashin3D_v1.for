c-----------------------------------------------------------------
c v1 = critical strain and stress are not absolute values
c this causes d>1 becaus of temp value get negative
c-----------------------------------------------------------------
      subroutine vumat (
C Read only -
...
c if all damage variables are one, delete the element. 
      if (sum(stateNew(k,7:12)) .eq. six) then
        stateNew(k,29)=one
